...
\end{Verbatim}
 ...
 固定化はインスタンスごとに行う必要がある。\ref{SaftyForInstance}節では
 別の方法を解説する。
...
タを安全に管理する方法としては\ES で導入された\ElmJ{WeakMap}を用いる方法
がある。
...
\begin{Exec}\upshape\label{WeakMap}
  次の例は実行例\ref{PersonWidthGetter}を\ElmJ{WeakMap}を利用して、
 \ElmJ{extensible}属性を使用しないで値の保護をするものである。
\begin{Verbatim}[numbers=left]
...
{}
\end{Verbatim}
インスタンスのプロパティは空である。これはインスタンスのプロパティを設定していな
 いためである。結果を展開するとメソッドが見える。
