...
\begin{itemize}
 ...
 \item 38行目と39行目ではそれぞれ年、月のプルダウンメニューを作成して
...
イベント処理の前半部分である最上位のオブジェクトからイベントが発生し
たオブジェクトにイベントの発生が伝播する段階を
\keyitem{}といい、後半の部分のイベントが発生し
...
次のリストは上のHTMLファイルから読み込まれるCSSファイルのものである。
\listinginput{1}{event.css}
このスタイルシートは各要素の大きさや配置について指定している。
%\listinginput{1}{event.js}
\begin{listing}{1}
window.onload = function() {
  var Squares   = document.getElementById("Squares");
  var Select    = document.getElementById("select");
