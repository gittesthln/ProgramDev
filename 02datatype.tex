...
1文字だけの文字の型はない。\ES では文字列の中に式の値を埋め込むことがで
きるテンプレートリテラルが新たに導入された。テンプレートリテラルはバック
クオート(\Verb+`+)ではさむ。埋め込む式は\texttt{\$\{2+3\}}のように記述する。
\begin{Verbatim}
`2+3=${2+3}`  => "2+3=5"
\end{Verbatim}
...
\begin{itemize}
 ...
 \item 変数の宣言は \texttt{let} で行う。\texttt{var}でもできるが、問題
       があるのでこの授業では使用しない。
 \item 宣言時に初期化ができる。 ES2015 では初期化以降で値の変更が
       できない\texttt{const}で変数の宣言ができる。
...
>b[3]= b[3]*10;      // 4番目の要素を10倍
30
>b;
(5) [0, 1, 2, 30, 4]
>a;
(5) [0, 1, 2, 30, 4] // a も b と同じ要素の値を持つ
\end{Verbatim}
...
\end{Verbatim}
  ...
  この機能は関数の戻り値が配列であるとき、戻り値の必要なところだけ利用す
       るために便利な機能である。
...
\begin{center}
 ...
 メンバー&\multicolumn{1}{c|}{説明} \\\hline
  ...
  \Verb+indexOf(value,start)+&配列の要素で\texttt{value}に等しい
      ...
      \\ \hline
  \Verb+lastIndexOf(value,start)+&配列の要素で\texttt{value}に等しいものを探
      し、その位置を返す。見つからない場合は\texttt{-1}を返す。
...
Java やC++のように
\ElmJ{for}分の初期化のところに現れる変数を\ElmJ{let}を用いて宣言すること
