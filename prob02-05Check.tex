...
\newcommand{\WW}{25zw}
\newcommand{\WWii}{c}
\ifAnswer\else\ifText%
\renewcommand{\WW}{35zw}
\renewcommand{\WWii}{m{4zw}}
\fi\fi
\newcommand{\Rule}{\ifAnswer\else%
   \ifText\rule[-2.8ex]{0em}{6.8ex}\else\rule[-2ex]{0em}{5ex}\fi\fi}
\ifAnswer
\newcommand{\AnsShow}[3]{\texttt{#1}&#2&#3\\\hline}
 \begin{tabular}{|>{\Rule}c|c|m{25zw}|}\hline
\else
  \newcommand{\AnsShow}[3]{\texttt{#1}&&\\\hline}
  \ifText
  \begin{tabular}{|>{\Rule}c|m{4zw}|m{35zw}|}\hline
   \else
 \begin{tabular}{|>{\Rule}c|c|m{25zw}|}\hline
  \fi
\fi
...
%  \AnsShow{5\%3}{$2$}{$5$を$3$で割った余り}
  ...
  \AnsShow{4-"0xff"}{\texttt{$-251$}}{\texttt{"0xff"}が$255$と解釈されて
  計算される。}
  ...
  \AnsShow{4+parseInt("ff",16)}{$259$}{基数を$16$と指定しているので、正しく$255$と解
	  釈される。}
  \AnsShow{4+"1e1"}{\texttt{"4+1e1"}}{1番目と同じ}
\AnsShow{4+parseInt("1e1")}{$5$}{"1e1" は数値リテラルとしては
  ...
  \AnsShow{false == undefined}{\texttt{false}}{\texttt{false}が数に$0$に変
          換され、数と\texttt{undefined}は等しくない。}
