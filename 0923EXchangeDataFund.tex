...
\section{Web Storage}
HTML5から独立した Web Storage は localStoarage と sessionStorage の2種類
がある。localStorage は文字列をキーに、文字列の値を持つ Storage オブジェクト
である。同一の出身(プロトコルやポート番号も含む)のすべてのドキュメント
がおなじlocalStorageを共有する。このデータは意識的に消さない限り存在する。
