...
Git の解説はWebを検索すればいくつか見ることができる。公式の解説書は

\Verb+https://git-scm.com/book/ja/v2+

にある。1章と2章を参考に使い始めるのがよいであろう。
\subsection{初期設定}
ローカルでいくつかの準備が必要となる。

\subsubsection{ユーザーネームの登録}
...
表\ref{Gitfundamental}は Git の基本操作をまとめたものである。
\begin{table}
\caption{Git の基本的なコマンド}\label{Gitfundamental}
\begin{center}
 \begin{tabular}{|c|c|c|}\hline
コマンド&パラメータ&説明\\\hline
\Verb+git init+ & &プロジェクトの初期化\\\hline
\Verb+git status+ &(\Verb+-s+) &プロジェクトの状態表示\\\hline
\Verb+git add+ & ファイルリスト&次回の\Verb+commit+へのファイルの登録 \\\hline
\Verb+git commit+& \Verb+-m+ コメント& リポジトリへの変更の登録\\ \hline
\Verb+git remote add+ &短縮名 リポジトリ & リポジトリの短縮名の登録\\
 \hline
\Verb+git rm+ & &リポジトリからファイルを削除することの登録 \\ \hline
